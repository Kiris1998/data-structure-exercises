...
int findKthLargestNum(int *nums,int numsSize,int k)  //nums为要查找的数组，k为第k个最大的数
{
	srand(time(0)); 
	return quickSelect(nums,0,numsSize - 1,numsSize - k);
}

int quickSelect(int *nums,int min,int max,int index)  //nums为要查找的数组,min为划分后数组的左坐标，max为对应右坐标，index为要找的第k个最大数
{
...
int randNum(int *nums,int min,int max)  //去一个随机数做中间元，其他参数与上一致
{
...
int divideQue(int *nums,int min,int max) //将小于随机数对应数的全放入数组最左侧，不管是否排好序，其他参数与上一致
{
	int i = min - 1;  //i为左指针
